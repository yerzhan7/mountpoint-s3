...
impl Executor {
    pub fn new(global: &GlobalConfig) -> Result<Self, ExecutionError> {
        ...
        let max_memory_target = global.max_memory_target.unwrap_or_else(|| {
            let detected = ((effective_total_memory() as f64 * 0.95) / (1024.0 * 1024.0)) as usize;
            eprintln!("No max_memory_target configured; auto-detected: {} MiB (95% of effective memory)", detected);
            detected
        });

        eprintln!("Memory target: {} MiB", max_memory_target);
